/**
 * This file was auto-generated by Fern from our API Definition.
 */

export interface TelephonyGetResponse {
    /** The number called. */
    to_number?: string;
    /** Caller ID. */
    from_number?: string;
    /** The unique ID of a published project in AI Studio. Can be obtained in AI Studio - Project - Release Management. */
    pipeline_id?: string;
    /**
     * Call type:
     * - `inbound`: Inbound call.
     * - `outbound`: Outbound call.
     */
    type?: TelephonyGetResponse.Type;
    /** A unique identifier for the call session. */
    agent_id?: string;
    /** RTC channel name. */
    channel?: string;
    /**
     * Reason for call ending:
     * - `request`: Actively hung up.
     * - `hangup`: The other party hung up.
     * - `failed`: Call failed.
     */
    reason?: TelephonyGetResponse.Reason;
    /** Call creation timestamp (in seconds). */
    create_ts?: number;
    /**
     * Call status:
     * - `answered`: The call was answered.
     * - `hangup`: The call was disconnected.
     */
    state?: TelephonyGetResponse.State;
    /** Call end timestamp (in seconds). Returns `0` or `null` if the call has not ended. */
    stop_ts?: number;
}

export namespace TelephonyGetResponse {
    /**
     * Call type:
     * - `inbound`: Inbound call.
     * - `outbound`: Outbound call.
     */
    export type Type = "inbound" | "outbound";
    export const Type = {
        Inbound: "inbound",
        Outbound: "outbound",
    } as const;
    /**
     * Reason for call ending:
     * - `request`: Actively hung up.
     * - `hangup`: The other party hung up.
     * - `failed`: Call failed.
     */
    export type Reason = "request" | "hangup" | "failed";
    export const Reason = {
        Request: "request",
        Hangup: "hangup",
        Failed: "failed",
    } as const;
    /**
     * Call status:
     * - `answered`: The call was answered.
     * - `hangup`: The call was disconnected.
     */
    export type State = "answered" | "hangup";
    export const State = {
        Answered: "answered",
        Hangup: "hangup",
    } as const;
}
