...
Filler text can also be generated by an OpenAI-compatible LLM while the main
LLM is processing the user's message. The generated configuration is optional;
omitted fields use the service defaults. The phase-one fallback strategy is
`static`.
